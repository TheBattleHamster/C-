...
TexturePtr load_texture(const char * filename) {
	...
	if (surf == nullptr)
		SDL_DIE("IMG_Load()");
	TexturePtr result;
	result = TexturePtr(SDL_CreateTextureFromSurface(ren.get(), surf),
			SDL_DestroyTexture);
	if (result == nullptr)
		SDL_DIE("SDL_CreateTextureFromSurface()");
...
void render_bird() {
	...
	SDL_RenderCopy(ren.get(), bird.get(), nullptr, &r_bird);
}
...
void render_cat() {
	...
	SDL_RenderCopyEx(ren.get(), cat.get(), nullptr, &r_cat, 0., nullptr, flip);
}
...
void render_egg() {
	...
	SDL_RenderCopy(ren.get(), egg1.get(), nullptr, &r_egg);
}
int omlet_x = 0, omlet_y = 10, omlet_w = 73, omlet_h = 56, omlet_visible = 0;
void render_omlet() {
	if (!omlet_visible)
		return;
	if ((omlet_visible <= 20) && ((omlet_visible % 2) == 1))
		return;
	SDL_Rect r_omlet { omlet_x, omlet_y, omlet_w, omlet_h };
	SDL_RenderCopy(ren.get(), egg2.get(), nullptr, &r_omlet);
}
